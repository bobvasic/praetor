...
// Raptor Labs neon-crimson palette. Solana brand tones (purple/green) are
// muted into deep crimson so the scene stays brand-pure.
const palette = {
  obsidian: "#0A0A0A",
  navy: "#0F0F0F",
  gold: "#FF4040",
...
function PraetorCityFrame({ recorderMode = false }: { recorderMode?: boolean }) {
  ...
  const render = useCallback((canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, time: number) => {
    ...
    bg.addColorStop(0, "rgba(36,8,8,1)");
    ...
    for (let i = 0; i < 36; i += 1) {
      ...
      drawLine(ctx, [[width * 0.18, y], [width * 0.82, y + Math.sin(i + time) * 2]], `rgba(255,26,26,${alpha})`, Math.max(1, dpr * 0.6));
    }
    ...
    roadLoops.forEach((scale, index) => {
      ...
      ctx.strokeStyle = index % 2 ? "rgba(255,64,64,0.42)" : "rgba(255,26,26,0.48)";
    ...
    sortedTowers.forEach((tower, index) => {
      ...
      if (tower.tone === "gold") {
        body.addColorStop(0, "rgba(64,8,8,0.96)");
        body.addColorStop(0.48, `rgba(255,64,64,${shimmer})`);
        body.addColorStop(1, "rgba(255,200,200,0.82)");
      } else if (tower.tone === "silver") {
        body.addColorStop(0, "rgba(40,12,12,0.94)");
        body.addColorStop(0.5, `rgba(245,245,245,${shimmer * 0.82})`);
        body.addColorStop(1, "rgba(120,40,40,0.86)");
      } else {
        body.addColorStop(0, "rgba(20,8,8,0.96)");
        body.addColorStop(0.58, `rgba(180,60,60,${shimmer * 0.7})`);
        body.addColorStop(1, "rgba(8,8,8,0.96)");
      }
      ctx.save();
      ctx.beginPath();
      ...
      ctx.shadowColor = tower.tone === "gold" ? "rgba(255,64,64,0.5)" : "rgba(255,26,26,0.22)";
      ctx.fill();
      if (index % 2 === 0) {
        drawLine(ctx, [[topX - towerWidth * 0.34, topY + towerWidth * 0.18], [baseX - towerWidth * 0.4, baseY]], "rgba(255,200,200,0.18)", dpr * 0.7);
      }
      ctx.restore();
    });
    ...
    for (let ring = 0; ring < 4; ring += 1) {
      ...
      ctx.strokeStyle = ring === 2 ? `rgba(255,26,26,${shieldPulse})` : `rgba(255,64,64,${0.22 + shieldPulse * 0.18})`;
    ...
    for (let i = 0; i < 8; i += 1) {
      ...
      drawLine(ctx, [start, [ix, iy]], `rgba(255,26,26,${0.12 + attackT * 0.5})`, dpr * 1.1, 10 * dpr);
      if (attackT > 0.72) {
        ctx.save();
        ctx.fillStyle = `rgba(255,64,64,${1 - attackT})`;
...
export function PraetorHeroScene({ className = "", recorderMode = false }: PraetorHeroSceneProps) {
  return (
    <div className={`relative h-full min-h-[420px] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-black ${className}`}>
      ...
      <div className="pointer-events-none absolute left-5 top-5 rounded-full border border-primary/40 bg-black/40 px-4 py-2 font-mono text-[10px] font-black uppercase tracking-[0.24em] text-primary backdrop-blur-md glow-box">
