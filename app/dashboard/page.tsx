                ...
                <HoverLift key={i} delay={i * 0.05}>
                  ...
                  {i === 1 && (
                    <Tooltip content="Protected Addresses are monitored Solana addresses where privileged or treasury actions are evaluated by Praetor policy.">
                      <div
                        tabIndex={0}
                        className="rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-arctic/70"
                      >
                        <MetricCard
                        ...
                        {protectedAddresses.map((item, index) => (
                          <FadeUp key={item.address} delay={index * 0.04}>
                            <div className="rounded-2xl border border-titanium/[0.10] bg-obsidian/[0.58] p-5 transition hover:border-arctic/[0.22] hover:bg-arctic/[0.045]">
                              ...
                              <p className="mt-3 text-sm text-titanium/[0.68]">
                                {item.policy}
                              </p>
                            </div>
                          </FadeUp>
                        ...
                        {signals.map((signal) => {
                          ...
                          return (
                            <Tooltip key={signal.label} content={signal.tip}>
                              <div
                                tabIndex={0}
                                className="rounded-2xl border border-arctic/[0.16] bg-arctic/[0.08] p-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-arctic/70"
                              >
                                <Icon
                                  className="h-5 w-5 text-arctic"
                                  aria-hidden
                ...
                <div className="grid gap-6 md:grid-cols-[0.8fr_1.2fr] md:items-center">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    {["Detect", "Attest", "Block"].map((item) => (
                      <div
                        key={item}
                        className="rounded-2xl border border-titanium/[0.10] bg-obsidian/[0.52] p-4"
                      >
